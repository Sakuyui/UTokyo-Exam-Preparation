...
namespace UTokyo.Structure.Graph
{
    ...
    {
        public static int MaxFlow(MatrixGraph<(int, int)> matrixGraph,int s, int t)
        {
            var mflow = 0;
        ...
        public static int MaxFlowDfs(MatrixGraph<(int flow, int capacity)> matrixGraph,bool[] used, int v, int t, int f)
        {
            //DFS套路，，判断叶子，处理访问记录
            //判断叶子
            if (v == t)
            ...
            //DFS套路-遍历所有有效分支节点
            var ext = matrixGraph.GetExtendNodes(matrixGraph[v]).Where(e => 
                (((int f, int c))e.Data).c > 0 && !used[matrixGraph[e]]).ToList();
            ...
            for (var i = 0; i < ext.Count; i++)
            {
                ...
                //开始DFS，是要找最小可用容量
                var d = MaxFlowDfs(matrixGraph, used,
                        edge.To, t, System.Math.Min(f, cap)); //要传递最小容量惹
        ...
        public static int MinCostFlow(MatrixGraph<int> matrixGraph, int s, int t, int f)
        {
            var n = matrixGraph.NodeCount;
                ...
                while (update)
                {
                    ...
                    for (var v = 0; v < n && dist[v] != int.MaxValue; v++)
                    {
                        ...
                        for (var i = 0; i < ext.Count; i++)
                        {
                            ...
                            prevv[edge.To] = v;
                            preve[edge.To] = matrixGraph[ext[i]];
                            update = true;
                        }
                    }
                }
